<template>
  <div class="table-container">
    <vab-query-form>
      ...
      <vab-query-form-right-panel>
        <el-form ref="form" class="list-form" :inline="true" :model="queryForm" @submit.native.prevent>
          <el-form-item>
            <el-input v-model="queryForm.data.name" placeholder="菜单名称" />
          </el-form-item>
    ...
    >
      ...
      <el-table-column label="父级标题" prop="parentTitle" show-overflow-tooltip />
      <el-table-column label="组件" prop="component" show-overflow-tooltip />
      <el-table-column label="路径" prop="path" show-overflow-tooltip />
      <el-table-column label="重定向路径" prop="redirect" show-overflow-tooltip />
      <el-table-column label="比重" prop="order" show-overflow-tooltip />
      <el-table-column label="是否删除" show-overflow-tooltip>
        <template #default="scope">
          <el-tag v-if="scope.row.menuIsDeleted == 0">未删除</el-tag>
  ...
  export default {
    ...
    methods: {
      ...
      async fetchData() {
        ...
        const { data, count } = await getMenus(this.queryForm)
        this.list = data
        this.total = count
